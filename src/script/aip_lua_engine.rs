...
pub struct AipLuaEngine {
	lua: Lua,
}

impl core::fmt::Debug for AipLuaEngine {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		f.debug_struct("AipLuaEngine").finish()
	}
}
			...
			let func = match entry.kind {
				...
				AipFnKind::Async => {
					...
					lua.create_async_function(move |lua: Lua, args: MultiValue| {
						let arg = args.into_iter().next().unwrap_or(Value::Nil);
						// The async handler closure yields a `Send` serde_json::Value (Lua is never captured
						// into the future). Perform the final serde -> Lua conversion here, on the Lua thread,
						// after awaiting the handler future.
