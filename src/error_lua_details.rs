...
impl LuaErrorDetails {
	/// Create the details from the full script, an optional failing line number, and the message.
	///
	/// `surround_code` is derived from `script` and `line_number` at construction time.
	pub fn new(script: impl Into<Arc<str>>, line_number: Option<u32>, message: impl Into<String>) -> Self {
	...
	pub fn from_lua_error(lua_error: &mlua::Error, script: impl Into<Arc<str>>) -> Self {
		...
		Self::new(script, line_number, buff.join("\n"))
	}
}
...
mod tests {
	...
	fn test_error_lua_details_new_surround_code() -> Result<()> {
		...
		// -- Exec
		let details = LuaErrorDetails::new(script, Some(3), "some failure message");
	...
	fn test_error_lua_details_new_no_line() -> Result<()> {
		...
		// -- Exec
		let details = LuaErrorDetails::new(script, None, "plain message");
	...
	fn test_error_lua_details_display_with_line() -> Result<()> {
		// -- Setup & Fixtures
		let script = "local a = 1\nlocal b = bad()\nreturn a";
		let details = LuaErrorDetails::new(script, Some(2), "aip.test - some failure");
	...
	fn test_error_lua_details_display_no_line() -> Result<()> {
		// -- Setup & Fixtures
		let details = LuaErrorDetails::new("local a = 1", None, "plain message");
